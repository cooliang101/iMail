...
CREATE TABLE IF NOT EXISTS developer_tokens (
  id TEXT PRIMARY KEY, user_id TEXT NOT NULL DEFAULT '__legacy__', name TEXT NOT NULL, token_hash TEXT NOT NULL UNIQUE, prefix TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS translation_provider_profiles (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL,
  display_name TEXT NOT NULL,
  execution_target TEXT NOT NULL CHECK (execution_target IN ('web-view', 'local-service', 'remote-service')),
  ...
  CHECK ((credential_kind IS NULL) = (encrypted_credential IS NULL)),
  CHECK ((consent_revision IS NULL) = (consent_accepted_at IS NULL))
) STRICT;
CREATE INDEX IF NOT EXISTS translation_provider_profiles_owner
  ON translation_provider_profiles(user_id, created_at, id);
CREATE TABLE IF NOT EXISTS message_translation_cache (
  ...
  translated_segments_json TEXT NOT NULL CHECK (json_valid(translated_segments_json)),
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL,
  PRIMARY KEY (user_id, message_id, body_hash, source_language, target_language,
               profile_id, provider_revision, segment_version)
) STRICT;
CREATE INDEX IF NOT EXISTS message_translation_cache_owner
  ON message_translation_cache(user_id, updated_at DESC);
