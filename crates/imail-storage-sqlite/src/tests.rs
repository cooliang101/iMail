...
impl Fixture {
    fn new(schema_version: u32) -> Self {
        ...
        connection.execute_batch(&format!(r#"
          ...
          CREATE TABLE message_sources (message_id TEXT PRIMARY KEY REFERENCES messages(id) ON DELETE CASCADE, source BLOB NOT NULL);
          ...
          INSERT INTO message_sources VALUES ('m1', X'46726F6D3A2073656E646572406578616D706C652E636F6D0D0A0D0A426F6479');
...
fn raw_message_sources_are_exact_and_owner_scoped() {
    ...
    assert_eq!(
        ...
        Some(b"From: sender@example.com\r\n\r\nBody".to_vec())
    );
    assert_eq!(store.message_source("other-user", "m1").unwrap(), None);
    assert_eq!(store.message_source("user-1", "missing").unwrap(), None);
}
...
fn reads_public_models_without_exposing_encrypted_values_or_token_hashes() {
    ...
    assert_eq!(inventory.schema_version, 17);
...
fn composition_preferences_are_persistent_validated_and_owner_scoped() {
    ...
    let owner = store
        .create_user("composition-owner", "Owner", "composition-password-123")
        .unwrap();
    let connection = Connection::open(&database).unwrap();
    connection
        .execute(
            "UPDATE accounts SET user_id=?1 WHERE user_id='user-1'",
            [&owner.id],
        )
        .unwrap();
    drop(connection);
    let patch: AppPreferencesPatch = serde_json::from_value(json!({"composition":{
        "signatures":[{"accountId":"a-first","text":"Thanks\nOwner","newMessages":true,"replies":false}],
        "templates":[{"id":"follow-up","name":"Follow up","subject":"Checking in","text":"Hello"}]
    }})).unwrap();
    let updated = PreferencesService::new(&mut store)
        .update(&owner.id, patch.clone())
        .unwrap();
    assert_eq!(updated.composition.signatures[0].text, "Thanks\nOwner");
    let foreign = PreferencesService::new(&mut store)
        .update("outsider", patch.clone())
        .unwrap_err();
    assert_eq!(foreign.code(), "PREFERENCES_COMPOSITION_INVALID");
    assert!(PreferencesService::new(&mut store)
        .read("outsider")
        .unwrap()
        .composition
        .is_empty());
    let mut invalid = patch;
    invalid.composition.as_mut().unwrap().templates[0]
        .name
        .clear();
    assert!(PreferencesService::new(&mut store)
        .update(&owner.id, invalid)
        .is_err());
    drop(store);
    let mut store = SqliteAuthStore::open_database(&database).unwrap();
    assert_eq!(
        PreferencesService::new(&mut store)
            .read(&owner.id)
            .unwrap()
            .composition,
        updated.composition
...
#[test]
fn v12_backfills_headers_from_exact_source_and_preserves_draft_envelopes() {
    ...
    connection
        .execute(
            "UPDATE message_sources SET source=?1 WHERE message_id='m1'",
            [source.as_slice()],
        )
        .unwrap();
    drop(connection);
    let report = migrate_database(&database).unwrap();
    assert_eq!(report.applied_versions, [12, 13, 14, 15, 16, 17]);
    let mut store = SqliteAuthStore::open_database(&database).unwrap();
    let message = store.message("user-1", "m1").unwrap().unwrap();
    assert_eq!(message.headers.cc[0].address, "copy@example.com");
    assert_eq!(message.headers.reply_to[0].address, "replies@example.com");
    assert_eq!(message.headers.reply.in_reply_to, ["<parent@example.com>"]);
    assert_eq!(
        message.headers.reply.references,
        ["<root@example.com>", "<parent@example.com>"]
    );
    assert_eq!(
        store.message_source("user-1", "m1").unwrap().unwrap(),
        source
    );
    let draft: DraftReadModel = serde_json::from_value(json!({
        "id":"bcc-draft","accountId":"a-first","to":[],"cc":[],"bcc":["hidden@example.com"],
        "inReplyTo":["<parent@example.com>"],"references":["<root@example.com>"],
        "subject":"Reply","text":"Body","html":"<p>Body</p>","attachments":[],
        "createdAt":"2026-08-31T00:00:00Z","updatedAt":"2026-08-31T00:00:00Z"
    }))
    .unwrap();
    store.upsert_draft("user-1", &draft).unwrap();
    drop(store);
    let store = SqliteAuthStore::open_database(&database).unwrap();
    assert!(store.list_drafts("outsider").unwrap().is_empty());
    assert_eq!(
        store
            .list_drafts("user-1")
            .unwrap()
    ...
    assert_eq!(store.conversation_candidates("user-1").unwrap()[0].text, "");
    assert!(store
        .conversation_candidates("outsider")
        .unwrap()
        .is_empty());
}
...
fn rust_backup_round_trips_and_is_accepted_by_the_node_restore_preflight() {
    ...
    assert_eq!(node_report["schemaVersion"], 17);
...
#[test]
fn rust_migrates_v10_to_v11_with_exact_message_source_storage() {
    let fixture = raw_migration_fixture(include_str!("../sql/schema-v10.sql"));
    ...
    assert_eq!(report.applied_versions, [11, 12, 13, 14, 15, 16, 17]);
    let connection = Connection::open(fixture.root.join("imail.sqlite")).unwrap();
    let exists: bool = connection
        .query_row(
            "SELECT EXISTS(SELECT 1 FROM sqlite_master WHERE type='table' AND name='message_sources')",
            [],
            |row| row.get(0),
        )
        .unwrap();
...
#[test]
fn rust_migrates_v8_to_v11_and_encrypts_translation_provider_credentials() {
    ...
    assert_eq!(report.to_version, 17);
    assert_eq!(report.applied_versions, [9, 10, 11, 12, 13, 14, 15, 16, 17]);
...
#[test]
fn rust_migrates_v16_to_v17_with_mail_work_queue_table() {
    ...
    let exists: bool = connection
        .query_row(
            "SELECT EXISTS(SELECT 1 FROM sqlite_master WHERE type='table' AND name='mail_work_items')",
            [],
            |row| row.get(0),
        )
        .unwrap();
...
#[test]
fn rust_migrates_v2_sync_schema_to_v11_without_losing_valid_rows() {
    let fixture = raw_migration_fixture(
        r#"
...
fn rust_migration_serializes_concurrent_openers_and_rechecks_version() {
    ...
    assert!(reports.iter().any(|report| report.from_version == 17));
    assert!(reports.iter().all(|report| report.to_version == 17));
}
