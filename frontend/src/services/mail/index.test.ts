...
describe('mail service client adapters', () => {
  ...
  it('maps every remaining UI domain and rejects an opaque fallback route', () => {
    const mapped: Array<[string, RequestInit | undefined, string]> = [
      ...
      ['/api/mail-work-items?status=needsReply', undefined, 'mailWorkItemsList'],
      ['/api/messages/m1/work-item', { method: 'PUT', body: '{"status":"needsReply"}' }, 'mailWorkItemSet'],
      ['/api/messages/m1/work-item', { method: 'DELETE' }, 'mailWorkItemComplete'],
      ['/api/messages/m1/reply-draft', { method: 'POST', body: '{"mode":"replyAll","text":"收到"}' }, 'mailReplyDraftCreate'],
      ['/api/drafts/draft-1/schedule', { method: 'POST', body: '{"confirmed":true,"sendAt":"2026-09-02T08:00:00Z","requestId":"request-1"}' }, 'mailDraftSchedule'],
