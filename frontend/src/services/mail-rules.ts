...
export const mailRulesService = {
  ...
  get: (id: string) => api<{ rule: MailRule }>(`/api/mail-rules/${encodeURIComponent(id)}`),
  ...
  setEnabled: (id: string, enabled: boolean) => api<{ rule: MailRule }>(`/api/mail-rules/${encodeURIComponent(id)}/enabled`, { method: 'PATCH', body: JSON.stringify({ enabled }) }),
