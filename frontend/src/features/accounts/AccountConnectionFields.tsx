...
export function AccountConnectionFields({ provider, usesOAuth, oauthConfigured, credentialGuide, workspaceOptions, busy, advanced, onAdvancedChange, onManualModeChange, onCancelOAuth }: {
  ...
  return <>
    ...
    {provider === 'custom' && <div className="advanced-settings"><button type="button" onClick={() => onAdvancedChange(!advanced)}><Gear size={17} />IMAP / SMTP 设置<CaretDown size={15} /></button>{(advanced || provider === 'custom') && <div className="form-grid"><label><span>IMAP 主机</span><AppInput name="imapHost" placeholder="imap.example.com" required /></label><label><span>IMAP 端口</span><AppInput name="imapPort" type="number" defaultValue="993" required /></label><label><span>IMAP 加密</span><AppSelect name="imapSecurity" defaultValue="implicit-tls" options={[{ value: 'implicit-tls', label: 'SSL / TLS' }, { value: 'starttls', label: 'STARTTLS' }]} /></label><label><span>SMTP 主机</span><AppInput name="smtpHost" placeholder="smtp.example.com" required /></label><label><span>SMTP 端口</span><AppInput name="smtpPort" type="number" defaultValue="465" required /></label><label><span>SMTP 加密</span><AppSelect name="smtpSecurity" defaultValue="implicit-tls" options={[{ value: 'implicit-tls', label: 'SSL / TLS' }, { value: 'starttls', label: 'STARTTLS' }]} /></label></div>}</div>}
