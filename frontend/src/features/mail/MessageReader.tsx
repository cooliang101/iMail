...
export function MessageReader({ message, account, accounts, onComposeConversationMessage, contacts, defaultBodyView, onReply, onReplyAll, onForward, onComposeSender, onFilterParticipant, onCloseMobile, onToggleFlag, onArchive, onDelete, onSnooze, onAddToWorkQueue, onManageLabels, onMarkUnread, onPrevious, onNext, onContextMenu, hasPrevious, hasNext, actionBusy }: {
  ...
  const [translationContextPoint, setTranslationContextPoint] = useState<{ x: number; y: number }>();
  ...
  useEffect(() => { setBodyView(defaultBodyView); setTranslationOpen(false); setTranslationMounted(false); setTranslationMode('bilingual'); setTranslationPresentation(undefined); setTranslationContextPoint(undefined); setRawMessageOpen(false); setBodyContextMenu(undefined); setBodyActionError(''); setBodyActionNotice(''); }, [defaultBodyView, message?.id]);
  ...
  return <article className="reader">
    ...
    {bodyContextMenu && <MessageBodyContextMenu
      ...
      onTranslate={() => { setTranslationContextPoint({ x: bodyContextMenu.x, y: bodyContextMenu.y }); setTranslationMounted(true); setTranslationOpen(true); }}
