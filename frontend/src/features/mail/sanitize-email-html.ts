...
const attributesByElement: Record<string, Set<string>> = {
  a: new Set(['href']), blockquote: new Set(['cite']), col: new Set(['span']), colgroup: new Set(['span']),
  img: new Set(['alt', 'height', 'src', 'title', 'width']), ol: new Set(['reversed', 'start', 'type']),
  q: new Set(['cite']), td: new Set(['colspan', 'rowspan']), th: new Set(['colspan', 'rowspan', 'scope']),
};
...
export function sanitizeEmailHtml(html: string, Parser: typeof DOMParser = DOMParser) {
  ...
  for (const element of elements) {
    ...
    if (blockedElements.has(tag)) { element.remove(); continue; }
    if (!allowedElements.has(tag)) { element.replaceWith(...Array.from(element.childNodes)); continue; }
    ...
    for (const attribute of Array.from(element.attributes)) {
      ...
      if (!permitted) { element.removeAttribute(attribute.name); continue; }
      if (name === 'style') {
...
function preserveLegacyPresentation(element: Element, tag: string) {
  ...
  if (align && ['left', 'right', 'center', 'justify'].includes(align)) {
    if (tag === 'table') {
      if (align === 'center') { setStyleFallback(element, 'margin-left', 'auto'); setStyleFallback(element, 'margin-right', 'auto'); }
      else if (align === 'left' || align === 'right') setStyleFallback(element, 'float', align);
    } else setStyleFallback(element, 'text-align', align);
  }
  const valign = element.getAttribute('valign')?.trim().toLocaleLowerCase();
  if (valign && ['baseline', 'bottom', 'middle', 'top'].includes(valign)) setStyleFallback(element, 'vertical-align', valign);
  if (tag === 'table') { const cellspacing = safeDimension(element.getAttribute('cellspacing')); if (cellspacing) setStyleFallback(element, 'border-spacing', cellspacing); }
}

function copyColorAttribute(element: Element, attribute: string, property: string) {
  const value = element.getAttribute(attribute)?.trim();
  if (value && /^(?:#[\da-f]{3,8}|(?:rgb|hsl)a?\([\d.%+\-,\s]+\)|[a-z]{1,24})$/i.test(value)) setStyleFallback(element, property, value);
}

function copyDimensionAttribute(element: Element, attribute: 'height' | 'width') { const value = safeDimension(element.getAttribute(attribute)); if (value) setStyleFallback(element, attribute, value); }
function safeDimension(value: string | null) { const normalized = value?.trim(); if (!normalized || !/^(?:auto|0|\d+(?:\.\d+)?(?:px|%|em|rem|pt|pc|in|cm|mm|vw|vh|vmin|vmax)?)$/i.test(normalized)) return ''; return /^\d+(?:\.\d+)?$/.test(normalized) && normalized !== '0' ? `${normalized}px` : normalized; }
function setStyleFallback(element: Element, property: string, value: string) { const style = (element as HTMLElement).style; if (!style.getPropertyValue(property)) style.setProperty(property, value); }

function sanitizeInlineStyle(value: string, document: Document) {
  const probe = document.createElement('span'); probe.setAttribute('style', value);
...
function normalizeCssForInspection(value: string) {
  return value.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\\([\da-f]{1,6})\s?/gi, (_match, code: string) => String.fromCodePoint(Number.parseInt(code, 16))).replace(/\\(.)/g, '$1').replace(/\s+/g, ' ').trim();
}

function sanitizeLink(element: Element) {
  const href = element.getAttribute('href');
  if (!href || !isSafeUrl(href, ['http:', 'https:', 'mailto:', 'tel:'])) { element.removeAttribute('href'); return; }
  element.setAttribute('target', '_blank'); element.setAttribute('rel', 'noopener noreferrer'); element.setAttribute('referrerpolicy', 'no-referrer');
}
...
function isSafeUrl(value: string, protocols: string[]) { try { return protocols.includes(new URL(value).protocol.toLocaleLowerCase()); } catch { return false; } }
function isSafeRasterDataUrl(value: string) { return /^data:image\/(?:avif|bmp|gif|jpeg|png|webp);base64,[a-z\d+/=\s]+$/i.test(value); }
