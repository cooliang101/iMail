...
export function BilingualHtmlMessageBody({ html, subject, presentation, onShowOriginal }: {
  html: string;
  subject: string;
  ...
  if (!result) return <BilingualMessageBody presentation={presentation} mode="bilingual" hasHtml onShowOriginal={onShowOriginal} />;

  return <div className="mail-bilingual-html-body" aria-busy={presentation.busy}>
    <div className="mail-bilingual-layout-note"><span>译文已按原始邮件排版显示在对应段落下方。</span><button type="button" onClick={onShowOriginal}>仅查看原始邮件</button></div>
