...
export function AuthorizationExport({ accountCount, setNotice }: { accountCount: number; setNotice: (notice: Notice) => void }) {
  ...
  return <section className="privacy-action-section">
    ...
    {!open ? <AppButton appearance="secondary" type="button" icon={<DownloadSimple size={17} />} disabled={accountCount === 0} onClick={() => setOpen(true)}>{accountCount === 0 ? '暂无可导出的邮箱' : `导出 ${accountCount} 个邮箱`}</AppButton>
      : <form className="privacy-action-form" onSubmit={submit}>
        ...
        <div className="privacy-action-buttons"><button type="button" onClick={() => { setOpen(false); setAcknowledged(false); setError(''); }} disabled={busy}>取消</button><AppButton appearance="primary" type="submit" icon={<DownloadSimple size={17} />} disabled={busy || !acknowledged}>{busy ? '正在加密…' : '加密并下载'}</AppButton></div>
      </form>}
  </section>;
}
