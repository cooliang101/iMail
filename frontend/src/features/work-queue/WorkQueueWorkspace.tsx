...
export function WorkQueueWorkspace({ items, accounts, selectedId, onSelect, onUpdate, onComplete, onOpenDraft }: {
  ...
  return <section className="message-pane work-queue-pane">
    <header className="draft-pane-header work-queue-header"><div><span>Agent 与人工协作</span><strong>邮件处理队列</strong><small>{items.length} 项待处理{overdue ? ` · ${overdue} 项已到期` : ''}</small></div><AppSelect aria-label="筛选处理状态" value={filter} options={[{ value: 'all', label: '全部状态' }, ...statusOptions]} onValueChange={(value) => setFilter(value as MailWorkStatus | 'all')} /></header>
    {visible.length === 0 ? <div className="draft-empty"><ClipboardText size={42} weight="duotone" /><h2>当前没有处理项目</h2><p>在邮件阅读页加入队列，或让 Agent 按规则整理待办。</p></div> : <div className="work-queue-list app-scrollbar">
      {visible.map(({ item, message }) => {
        ...
        return <article key={item.id} className={`work-queue-item ${selectedId === message.id ? 'is-selected' : ''}`} onClick={() => onSelect(message.id)}>
          <div className="work-queue-item-heading"><span>{account && <AccountProviderMark provider={account.provider} />}{message.from.name || message.from.address}</span><em className={`work-queue-status is-${item.status}`}>{statusCopy[item.status]}</em></div>
