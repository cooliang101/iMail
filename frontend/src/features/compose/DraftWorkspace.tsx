...
export function DraftWorkspace({ drafts, remoteDrafts, accounts, selectedRemoteId, onOpen, onOpenRemote, onDelete, onCreate }: {
  ...
  return <section className="message-pane draft-pane"><header className="draft-pane-header"><div><span>本地与邮箱草稿</span><strong>草稿</strong><small>{drafts.length} 封本地 · {remoteDrafts.length} 封邮箱草稿</small></div><AppButton appearance="primary" icon={<PencilSimple size={16} />} onClick={onCreate}>新建</AppButton></header>
