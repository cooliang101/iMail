...
export function AppAccountRail({ user, accounts, accountFilter, onSelect, onAdd, onSettings, onSwitchAccount, onContextMenu }: {
  ...
  return <aside className="account-rail" aria-label={t('邮箱账户')}>
    ...
    <div className="rail-accounts">
      <button title={t('聚合所有邮箱')} aria-label={t('聚合所有邮箱')} className={`rail-avatar rail-all ${accountFilter === 'all' ? 'active' : ''}`} onClick={() => onSelect()} onContextMenu={(event) => onContextMenu(event)}><Tray size={20} /></button>
    ...
    <button title={t('设置')} aria-label={t('打开设置')} className="rail-avatar rail-settings" onClick={onSettings}><Gear size={19} /></button>
  </aside>;
}
