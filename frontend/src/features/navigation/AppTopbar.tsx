...
export function AppTopbar({ sidebarCollapsed, sidebarOpen, search, searchPlaceholder = '搜索当前范围内的邮件', searchShortcut, searchInputRef, onToggleSidebar, onOpenMobileSidebar, onSearchChange, onNotifications }: {
  sidebarCollapsed: boolean; sidebarOpen: boolean; search: string; searchPlaceholder?: string; searchShortcut: string; searchInputRef: RefObject<HTMLInputElement | null>;
  ...
  return <header className="topbar">
    ...
    <AppInput className="search-box" contentBefore={<MagnifyingGlass size={18} />} contentAfter={<kbd>{searchShortcut}</kbd>} ref={searchInputRef} value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder={searchPlaceholder} aria-label={searchPlaceholder} />
