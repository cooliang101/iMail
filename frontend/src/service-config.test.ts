...
describe('service configuration', () => {
  ...
  it('uses local mode and the embedded service by default', () => {
    const storage = memoryStorage();
    expect(configuredServiceMode(storage)).toBe('local');
    expect(configuredServiceUrl(storage)).toBe(LOCAL_SERVICE_URL);
  });

  it('uses the embedded Rust service only inside the Tauri runtime', () => {
    vi.stubGlobal('window', { location: { origin: 'http://localhost' } });
    expect(embeddedTauriServiceEnabled()).toBe(false);
  ...
  it('normalizes HTTP service addresses and rejects unsafe URL forms', () => {
    ...
    expect(() => saveServiceSelection({ mode: 'remote', remoteUrl: 'http://127.0.0.1:8787' }, memoryStorage())).toThrow('远程');
