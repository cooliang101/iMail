...
function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className={`primary-sidebar ${sidebarOpen ? 'mobile-open' : ''}`}>
      ...
      {accounts.some((account) => account.mailboxes.some((mailbox) => mailbox.selectable && !['\\Inbox', '\\Sent', '\\Archive', '\\All'].includes(mailbox.specialUse ?? ''))) && <><div className="section-label"><span>邮箱文件夹</span></div><nav className="nav-block mailbox-nav">{accounts.map((account) => account.mailboxes.filter((mailbox) => mailbox.selectable && !['\\Inbox', '\\Sent', '\\Archive', '\\All'].includes(mailbox.specialUse ?? '')).map((mailbox) => <button key={`${account.id}:${mailbox.path}`} className={view === 'folder' && activeMailbox?.accountId === account.id && activeMailbox.path === mailbox.path ? 'active' : ''} onClick={() => selectMailbox(account.id, mailbox.path, mailbox.name)} title={`${account.displayName} · ${mailbox.path}`}><Folder size={16} /><span>{mailbox.name}<small>{account.displayName}</small></span><b>{mailbox.unread || ''}</b></button>))}</nav></>}
