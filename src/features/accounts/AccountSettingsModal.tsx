...
export function AccountSettingsModal({ accounts, onClose, onReload, setNotice }: { accounts: Account[]; onClose: () => void; onReload: () => Promise<void>; setNotice: (notice: Notice) => void }) {
  ...
  return <Overlay onClose={onClose} wide><section className="account-settings-modal">
    ...
    {accounts.length === 0 ? <div className="settings-empty"><Envelope size={38} weight="duotone" /><h3>还没有真实邮箱</h3><p>关闭设置后，点击左侧的加号接入第一个邮箱。</p></div> : <div className="settings-account-list">
      {accounts.map((account) => <article key={account.id} className={`settings-account-card ${editingId === account.id ? 'is-editing' : ''}`}>
        <header className="settings-account-summary"><i className={`provider-${account.provider}`}><ProviderIcon provider={account.provider} /></i><span><strong>{providerLabel[account.provider]} · {account.displayName}</strong><small>{account.email} · {account.group}</small><em className={`connection-${account.status}`}>{account.status === 'connected' ? '连接正常' : account.status === 'syncing' ? '正在同步' : account.lastError || '连接异常'}</em></span><small className="account-auth-kind">{account.authMethod === 'oauth2' ? 'OAuth 2.0' : '授权码 / 专用密码'}</small></header>
        {editingId === account.id ? <form className="account-profile-editor" onSubmit={(event) => void updateProfile(event, account)}><label><span>显示名称</span><AppInput name="displayName" defaultValue={account.displayName} maxLength={80} autoFocus required /></label><label><span>所属工作空间</span><AppSelect name="group" defaultValue={account.group} options={Array.from(new Set(accounts.map((item) => item.group))).map((group) => ({ value: group, label: group }))} /></label><div className="card-editor-actions"><button type="button" onClick={() => setEditingId(null)}>取消</button><Button appearance="primary" type="submit" disabled={busyId === account.id}>{busyId === account.id ? '保存中…' : '保存修改'}</Button></div></form>
