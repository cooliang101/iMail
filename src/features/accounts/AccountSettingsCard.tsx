...
import { ArrowClockwise, Globe, Key, PencilSimple, SlidersHorizontal, Trash } from '@phosphor-icons/react';
...
export function AccountSettingsCard({ account, section, syncStatus, workspaceOptions, busy, editing, proxyEditing, credentialOpen, removeConfirmOpen, syncEditing, onEdit, onCancelEdit, onUpdateProfile, onOpenProxy, onCloseProxy, onUpdateProxy, onRetry, onReconnect, onOpenCredential, onCloseCredential, onUpdateCredential, onOpenRemove, onCloseRemove, onRemove, onOpenSync, onCloseSync, onSaveSync, onQueueSync }: {
  ...
  editing: boolean; proxyEditing: boolean; credentialOpen: boolean; removeConfirmOpen: boolean; syncEditing: boolean;
  ...
  return <article className={`settings-account-card ${editing || proxyEditing ? 'is-editing' : ''} ${syncEditing ? 'is-sync-editing' : ''}`}>
    {section === 'accounts' && editing ? <form className="account-inline-editor" onSubmit={onUpdateProfile}>
    ...
    </form> : section === 'accounts' && proxyEditing ? <form className="account-inline-editor account-proxy-editor" onSubmit={onUpdateProxy}>
      ...
      <footer className="settings-account-actions card-editor-actions"><button type="button" onClick={onCloseProxy}>取消</button><Button appearance="primary" type="submit" disabled={busy}>{busy ? '正在验证…' : '验证并保存代理'}</Button></footer>
    </form> : <>
      <div className="settings-account-main"><header className="settings-account-summary"><i className={`provider-${account.provider}`}><ProviderIcon provider={account.provider} /></i><span><strong>{providerLabel[account.provider]} · {account.displayName}</strong><small>{account.email} · {account.group}</small><span className="account-status-line"><em className={`connection-${account.status}`}>{connectionText}</em>{section === 'accounts' && <em className={`account-proxy-status ${account.proxy ? 'is-enabled' : ''}`}><Globe size={12} />{proxyText}</em>}</span></span></header>{section === 'sync' && syncStatus && <AccountSyncSummary status={syncStatus} />}</div>
