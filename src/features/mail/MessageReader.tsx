...
import { Archive, ArrowBendUpLeft, ArrowBendUpRight, ArrowLeft, ArrowRight, Clock, Code, Envelope, Eye, File, Star, Tag, Trash } from '@phosphor-icons/react';
...
export function MessageReader({ message, account, defaultBodyView, onReply, onForward, onCloseMobile, onToggleFlag, onArchive, onDelete, onSnooze, onManageLabels, onMarkUnread, onPrevious, onNext, onContextMenu, hasPrevious, hasNext, actionBusy }: {
  ...
  return <article className="reader">
    <div className="reader-actions" onContextMenu={(event) => { event.preventDefault(); onContextMenu?.(message, { x: event.clientX, y: event.clientY }); }}><div><button className="mobile-reader-back" title="返回邮件列表" aria-label="返回邮件列表" onClick={onCloseMobile}><ArrowLeft size={18} /></button><button data-icon-tone="neutral" title="归档" aria-label="归档邮件" disabled={actionBusy || message.mailboxRole !== 'inbox'} onClick={onArchive}><Archive size={18} /></button><button data-icon-tone="danger" title="删除" aria-label="删除邮件" disabled={actionBusy} onClick={onDelete}><Trash size={18} /></button><button data-icon-tone="warning" title="稍后处理" aria-label="稍后处理" onClick={onSnooze}><Clock size={18} /></button><button data-icon-tone="info" title="管理标签" aria-label="管理邮件标签" onClick={onManageLabels}><Tag size={18} /></button></div><div>{message.html && <button type="button" className="reader-body-view-toggle" title={bodyView === 'source' ? '切换到渲染效果' : '切换到原始内容'} aria-label={bodyView === 'source' ? '切换到渲染效果' : '切换到原始内容'} onClick={() => setBodyView((current) => current === 'source' ? 'rendered' : 'source')}>{bodyView === 'source' ? <><Eye size={16} /><span>渲染效果</span></> : <><Code size={16} /><span>原始内容</span></>}</button>}<button title="上一封邮件" aria-label="上一封邮件" disabled={actionBusy || !hasPrevious} onClick={onPrevious}><ArrowLeft size={18} /></button><button title="下一封邮件" aria-label="下一封邮件" disabled={actionBusy || !hasNext} onClick={onNext}><ArrowRight size={18} /></button></div></div>
