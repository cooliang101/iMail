...
export function CreateTokenModal({ accounts, onClose, onCreated }: { accounts: Account[]; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (mailboxes.length === 0 && !scopes.includes('mcp:full')) { setError('普通 Token 请至少选择一个允许访问的邮箱；无邮箱时可签发 MCP 完整控制授权码'); setBusy(false); return; }
