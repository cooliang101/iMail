...
export function TokenWorkspace({ accounts, tokens, onCreate, onReload, setNotice }: { accounts: Account[]; tokens: DeveloperToken[]; onCreate: () => void; onReload: () => Promise<void>; setNotice: (notice: Notice) => void }) {
  ...
  return <section className="token-workspace"><header><div><span>本地开发能力</span><h1>邮件网关</h1><p>让本地项目或可信 Agent 用短期授权码安全操作邮箱，无需暴露已保存的邮箱凭据。</p></div><Button appearance="primary" icon={<Plus size={17} />} onClick={onCreate}>创建临时 Token</Button></header>
    <div className="endpoint-strip"><Code size={21} /><span><small>开发 API 地址</small><code>http://127.0.0.1:8787/gateway/v1</code></span><button onClick={() => void navigator.clipboard.writeText('http://127.0.0.1:8787/gateway/v1')}><Copy size={17} />复制</button></div>
    <div className="token-columns"><div className="token-list"><div className="token-title"><h2>有效 Token</h2><span>{tokens.filter((token) => token.expiresAt > new Date().toISOString()).length} 个正在生效</span></div>{tokens.length === 0 ? <div className="token-empty"><Key size={38} weight="duotone" /><h3>还没有临时 Token</h3><p>{accounts.length === 0 ? '可以先签发 MCP 完整控制授权码，让可信 Agent 帮你接入第一个邮箱。' : '创建一个给本地应用或可信 Agent 使用，原始值只显示一次。'}</p><button onClick={onCreate}>创建第一个 Token</button></div> : tokens.map((token) => { const mcp = token.scopes.includes('mcp:full'); return <article className="token-item" key={token.id}><div className="token-icon"><Key size={20} /></div><div><strong>{token.name}</strong><code>{token.prefix}••••••••••••</code><span>{token.scopes.map((scope) => scope === 'mcp:full' ? 'MCP 完整控制' : scope.replace('messages:', '')).join(' · ')} · {mcp ? '全部当前与未来邮箱' : `${token.mailboxes.length} 个邮箱`}</span></div><div className="token-time"><small>到期时间</small><span>{new Intl.DateTimeFormat('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(token.expiresAt))}</span></div><button className="revoke" onClick={() => void revoke(token.id)}>撤销</button></article>; })}</div>
      <aside className="quickstart"><div className="quickstart-title"><AddressBook size={21} /><div><strong>快速调用</strong><span>指定邮箱读取最新 10 封邮件</span></div></div><label className="gateway-account-select"><span>API 使用的邮箱</span><AppSelect value={gatewayAccount} onValueChange={setGatewayAccount} options={accounts.map((account) => ({ value: account.email, label: `${providerLabel[account.provider]} · ${account.displayName} · ${account.email}` }))} /></label><pre><code><span className="code-muted">curl</span> http://127.0.0.1:8787/gateway/v1/mailboxes/{gatewayAccount || 'user@example.com'}/messages?limit=10 \\{`\n`}  -H <span className="code-string">&quot;Authorization: Bearer imail_xxx&quot;</span></code></pre><div className="security-note"><WarningCircle size={18} /><p><strong>只使用邮箱地址</strong><span>网关请求和响应均不会暴露 iMail 内部邮箱 ID。</span></p></div><a href="/gateway/docs" target="_blank" rel="noreferrer">打开轻量 API Console <ArrowRight size={15} /></a></aside></div>
  </section>;
}
