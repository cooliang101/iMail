...
export function TokenWorkspace({ accounts, tokens, onCreateApi, onCreateMcp, onReload, setNotice }: { accounts: Account[]; tokens: DeveloperToken[]; onCreateApi: () => void; onCreateMcp: () => void; onReload: () => Promise<void>; setNotice: (notice: Notice) => void }) {
  ...
  const endpoint = absoluteServiceUrl(activeTab === 'mcp' ? '/mcp' : '/gateway/v1');
  ...
  return <section className="token-workspace">
    ...
    <div className={`external-access-toggle${activeEnabled ? ' is-enabled' : ''}`}><span><strong>{activeTab === 'mcp' ? '启用 MCP 接入' : '启用本地网关'}</strong><small>{activeEnabled ? '外部客户端可使用有效授权码连接' : '当前关闭，已有授权码也无法访问'}</small></span><AppCheckbox aria-label={activeTab === 'mcp' ? '启用 MCP 接入' : '启用本地网关'} checked={activeEnabled} disabled={!accessSettings || accessBusy} onChange={(_, data) => void setAccessEnabled(Boolean(data.checked))} /></div>

    <div className={`endpoint-strip${activeEnabled ? '' : ' is-disabled'}`}><Code size={21} /><span><small>{activeTab === 'mcp' ? 'Streamable HTTP 地址' : 'REST API 地址'}</small><code>{endpoint}</code></span><button disabled={!activeEnabled} onClick={() => void copyEndpoint(endpoint)}><Copy size={17} />复制</button></div>
