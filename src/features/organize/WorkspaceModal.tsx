...
export function WorkspaceModal({ accounts, onClose, onSaved }: { accounts: Account[]; onClose: () => void; onSaved: () => void | Promise<void> }) {
  ...
  return <Overlay onClose={onClose}><form className="utility-modal workspace-modal" onSubmit={submit}><div className="modal-header"><div><span>集中整理</span><h2>新增工作空间</h2><p>给一组邮箱设置相同的工作空间名称。</p></div><button type="button" aria-label="关闭工作空间窗口" onClick={onClose}><X size={21} /></button></div><label className="workspace-name"><span>工作空间名称</span><AppInput name="group" placeholder="例如：客户支持、开发测试" maxLength={40} required autoFocus /></label><fieldset><legend>包含的邮箱</legend>{accounts.map((account) => <label className="check-row" key={account.id}><AppCheckbox name="accountIds" value={account.id} /><i className={`provider-${account.provider}`}><ProviderIcon provider={account.provider} /></i><span><strong>{providerLabel[account.provider]} · {account.displayName}</strong><small>{account.email} · 当前：{account.group}</small></span><Check size={15} /></label>)}</fieldset>{error && <div className="inline-error"><WarningCircle size={17} />{error}</div>}<div className="modal-footer"><button type="button" onClick={onClose}>取消</button><Button appearance="primary" type="submit" disabled={busy}>{busy ? '保存中…' : '保存工作空间'}</Button></div></form></Overlay>;
}
