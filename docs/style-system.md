...
当前需要分层处理的典型路径包括：邮箱列表 → 单个邮箱（编辑/代理/凭据/移除在详情内展开）；隐私邮箱账户 → 地址管理/创建/Apple 授权；服务连接 → 远程服务；主题 → 自定义主题；隐私与数据 → 授权导出/数据清除。新增同等级复杂项时优先控制在两级；原地展开用于对象详情中的短表单和确认操作，不作为一级页面承载大型管理面板的手段。
