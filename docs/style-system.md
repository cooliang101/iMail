...
- 产品已经形成明确的冷灰绿色视觉语言：深色账户轨、浅色工作区、低饱和青绿主色。
...
2. **单一中性色族**：产品 UI 使用冷灰绿，禁止混入无语义的纯灰、暖灰或蓝紫渐变。
...
`main.tsx` 必须先导入 `theme.css`，再导入 `styles.css`。根 `FluentProvider` 使用 `data-theme="imail-light"` 标识当前主题。新增主题时复制完整的语义契约，例如 `[data-theme='imail-dark']`；不要在组件中判断主题并切换 class。
...
`--shadow-sm/md/lg` 均带冷绿色环境色，分别用于悬浮提示、菜单/浮层和弹窗。普通邮件行、设置条目和静态面板不使用阴影。
...
主题层已经独立，`styles.css` 暂时保留现有选择器以降低结构性回归风险。后续修改某个领域时，可把其规则原样迁到 `src/features/<domain>/<domain>.css`，由该 feature 的入口导入；跨领域基础样式保留在 `styles.css`。拆分只改变归属，不应同时重命名全部 class 或改变 API/交互。
