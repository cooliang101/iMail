...
账户轨的文字、弱图标、控制面、选中面、强调标记与边界分别使用 `--color-rail-text/muted/control/control-hover/control-active/control-active-text/accent/border`，悬浮层次使用 `--shadow-rail-control`。账户头像仍保留用户身份色，但选中轮廓和左侧标记必须跟随主题；禁止在账户轨组件中写死某套主题的薄荷色。
