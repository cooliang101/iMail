...
- 本地模式通过 Tauri 进程内 Rust 服务运行，不使用服务 URL、HTTP、Node 或独立 Worker。关闭窗口隐藏到托盘并继续同步，显式退出才停止。
...
1. 全新安装选择本地模式后，由嵌入式 Rust 完成身份检查；安装目录和进程树不含 Node/sidecar/manager/worker，8787 无 iMail listener。
...
9. 从“服务连接”打开应用日志：验证 `app.log` 包含本次启动、前端就绪、嵌入式服务和退出阶段；制造一个不含真实凭据的错误，确认邮箱、Bearer Token、OAuth code/state、Cookie 和密码均被脱敏。旧 `service.log` 只作为迁移证据保留。
