...
9. 从“服务连接”分别打开应用日志和服务日志：验证 `app.log` 包含本次启动、前端就绪和退出阶段，`service.log` 包含服务与 Worker 启动；制造一个不含真实凭据的前端错误与服务连接失败，确认错误被记录且测试邮箱、Bearer Token、OAuth code/state、Cookie 和密码均被替换为脱敏标记。应用日志达到 5 MB 后最多保留 3 份，服务日志轮转为 `service.log.1`。
