...
卸载桌面应用默认保留 `local-service/data`。“设置 → 通用”中的服务连接仅负责选择嵌入式本地或远程 Rust 服务，不提供数据删除。登录用户若进入“设置 → 隐私与数据 → 清除我的邮箱数据”，必须先核对范围，再提交当前 iMail 密码和固定确认文字；服务只清除该用户的数据，不删除登录账号、主密钥或其他用户的数据。
