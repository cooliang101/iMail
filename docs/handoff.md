...
- 设置中心可即时切换薄荷清新、石墨琥珀科技、蓝色商业和柔和粗野主义四套内置主题，也可编辑颜色、圆角、阴影和字体令牌，导入 AI JSON，并复制仓库中的生成规范。每套主题显式定义账户栏、邮件列表与正文阅读背景：账户栏分别采用森林光晕、琥珀终端网格、蓝图刻度和暖黄粉彩描边，柔和粗野主义邮件行使用三色轮换撞色。内置主题继续通过应用偏好同步；自定义主题保存在用户作用域的本地缓存。MCP 通过独立的 `theme_custom_get` / `theme_custom_update` 用户存储提供同一 JSON 协议，不扩展 HTTP 网关。
