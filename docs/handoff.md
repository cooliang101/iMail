...
- Windows NSIS 同时装配桌面前端、独立服务程序和不依赖 WebView 的服务管理程序；macOS ad-hoc DMG 可验证同一运行链。当前 CI 只保留 14 天的内部测试桌面产物，不创建公开 Release；Developer ID、Apple notarization、公开下载和自动更新属于延期的正式分发阶段。远程发布单元同时托管同版本 Web、API 和 Worker，并提供 Docker/Compose、备份恢复与升级预检工具。
