...
- 邮箱同步由独立 Worker 根据 SQLite 中的持久化策略和任务执行，不依赖前端、SSE 或开发者网关连接；收件箱显式保持 IMAP IDLE，断线自动重连，任务运行期间到达的通知会合并成一次可靠补跑。
- 桌面端提供显式的本地/远程服务选择。本地模式把随包服务程序注册为当前用户守护进程，桌面退出后继续同步；远程模式只连接用户部署的 HTTPS 实例，用于多设备共享，不做隐式迁移或故障回退。
- Windows NSIS 同时装配桌面前端、独立服务程序和不依赖 WebView 的服务管理程序；macOS ad-hoc DMG 可验证同一运行链。CI 保留 14 天的内部测试 Artifacts，并把 `main` 全门禁成功产物与 SHA-256 校验文件挂到 Draft Release，但不自动公开发布；Developer ID、Apple notarization、公开下载和自动更新属于延期的正式分发阶段。远程发布单元同时托管同版本 Web、API 和 Worker，并提供 Docker/Compose、备份恢复与升级预检工具。
