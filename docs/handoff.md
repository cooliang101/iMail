...
- 设置中心可即时切换经典薄荷清新、霓虹终端、深海蓝图和 Soft Neubrutalism 四套主题；主题与其他应用偏好一样按用户通过 HTTP/MCP 同步，并用本地缓存避免首屏闪烁。
...
- 新增主题必须同时维护 `src/features/appearance/theme-model.ts`、`src/theme.ts` 和 `src/theme.css`，并保留无效主题 ID 的安全回退。
