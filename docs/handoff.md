...
- 当前交付范围是 Windows 桌面端与服务端 Docker。Windows NSIS 同时装配桌面前端、独立服务程序和不依赖 WebView 的服务管理程序；Docker 发布单元同时托管同版本 Web、API 和 Worker，并提供 Compose、备份恢复与升级预检工具。原生 Linux 与 macOS 桌面构建不在支持范围。`0.0.1` 保留既有 Actions/Pre-release 历史证据，但当前 Actions 额度紧张，日常开发与内部测试改为本机验证和本机构建；普通分支推送与 pull request 不触发云端工作流，未经用户明确授权不得创建版本 tag 或手动触发。Windows 正式代码签名、公开下载和自动更新属于延期的正式分发阶段。
