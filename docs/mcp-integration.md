...
后台同步失败会同步更新账户的 `status` 与脱敏后的 `lastError`，桌面、HTTP 账户列表和 MCP `accounts_list` 使用同一份账户状态。OAuth 网络超时及临时服务错误会退避重试；明确的授权失效会暂停自动同步，需更新授权或成功验证连接后恢复。

同步成功后，账户恢复连接正常；其他文件夹遗留的网络或数据库错误只保留在文件夹同步状态中，不再把账户重新标为异常。仍未解除的 `authRequired` 状态继续保留在账户层面，避免掩盖需要处理的授权问题。
...
- 完整 EML / RFC 822 原始字节只通过已登录应用的 `GET /api/messages/:id/source` 按需读取，或通过 `GET /api/messages/:id/source/download` 原样下载；两者都不进入邮件列表、Gateway 或 MCP 响应，避免完整邮件头与附件内容被 Agent 授权面意外扩大。
- 邮件正文中的远程图片仅在用户显式选择保存时通过登录会话保护的 `POST /api/resources/image-download` 受控下载；服务拒绝内网地址、逐跳校验重定向，并限制图片类型和大小。该能力不进入 Gateway 或 MCP。
