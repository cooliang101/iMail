...
- 主题 ID 与回退由 `frontend/src/features/appearance/` 维护；新增内置主题必须同步 `frontend/src/theme.ts` 和 `frontend/src/theme.css`。自定义主题只接受安全令牌，字段变更须同步 `theme-runtime.ts`、`rust/crates/imail-core/src/theme.rs`、`rust/crates/imail-http/src/mcp.rs` 与 `docs/custom-theme.md`，不得扩展为任意 CSS，也不得加入 HTTP 网关。
